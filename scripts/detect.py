...
def main():
        ...
        try:
            ...
            if args.output and result.get("detections"):
                ...
                detector.visualize_detections(
                    image_path, result["detections"], vis_path
                )
    ...
    logger.info(
        f"Detection completed: {total_detections} wildlife found in {len(results)} images"
    )
    ...
    if all_species:
        ...
        for species, count in sorted(
            all_species.items(), key=lambda x: x[1], reverse=True
        ):
