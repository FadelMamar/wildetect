...
def main():
    ...
    prepare_parser = subparsers.add_parser('prepare-data', help='Prepare training data from FiftyOne')
    prepare_parser.add_argument('--dataset', required=True, help='FiftyOne dataset name')
    ...
    train_parser.add_argument('--data', required=True, help='Dataset configuration path')
    ...
    try:
        if args.command == 'prepare-data':
            ...
            if training_info['species_counts']:
                ...
                for species, count in sorted(training_info['species_counts'].items(), key=lambda x: x[1], reverse=True):
