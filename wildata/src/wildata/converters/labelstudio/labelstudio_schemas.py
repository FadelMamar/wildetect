...
class ResultValue(BaseModel):
    ...
    def to_normalized_bbox(self) -> Tuple[float, float, float, float]:
        ...
        """
        return (self.x / 100.0, self.y / 100.0, self.width / 100.0, self.height / 100.0)

    def to_coco_bbox(self, original_width: int, original_height: int) -> List[float]:
...
class Result(BaseModel):
    ...
    origin: Optional[ResultOrigin] = Field(
        default=None, description="Origin of the result"
    )
    score: Optional[float] = Field(
        default=None, description="Confidence score for predictions"
    )
...
class Prediction(BaseModel):
    ...
    model_version: Optional[str] = Field(
        default=None, description="Model version string"
    )
    created_ago: Optional[str] = Field(
...
class Annotation(BaseModel):
    ...
    completed_by: Optional[int] = Field(
        default=None, description="User ID who completed"
    )
    ...
    draft_created_at: Optional[datetime] = Field(
        default=None, description="Draft creation timestamp"
    )
    lead_time: Optional[float] = Field(
        default=None, description="Time spent annotating (seconds)"
    )
    prediction: Optional[Prediction] = Field(
        default=None, description="Associated prediction"
    )
    ...
    parent_prediction: Optional[int] = Field(
        default=None, description="Parent prediction ID"
    )
    parent_annotation: Optional[int] = Field(
        default=None, description="Parent annotation ID"
    )
    last_created_by: Optional[int] = Field(
        default=None, description="Last created by user ID"
    )
...
class Task(BaseModel):
    ...
    cancelled_annotations: int = Field(
        default=0, description="Cancelled annotation count"
    )
    ...
    unresolved_comment_count: int = Field(
        default=0, description="Unresolved comment count"
    )
    last_comment_updated_at: Optional[datetime] = Field(
        default=None, description="Last comment update timestamp"
    )
    project: Optional[int] = Field(default=None, description="Project ID")
    updated_by: Optional[int] = Field(default=None, description="User ID who updated")
    comment_authors: List[Any] = Field(
        default_factory=list, description="Comment authors"
    )
    ...
    def from_sdk_task(cls, sdk_task: "DataManagerTaskSerializer") -> "Task":
        ...
        if sdk_task.predictions:
            for pred in sdk_task.predictions:
                pred_dict = (
                    pred.model_dump() if hasattr(pred, "model_dump") else dict(pred)
                )
        ...
        return cls(
            ...
            comment_authors=list(sdk_task.comment_authors)
            if sdk_task.comment_authors
            else [],
        )
        ...
        for task in self.tasks:
            ...
            if filtered_annotations:
                ...
                task_dict["annotations"] = [
                    a.model_dump() for a in filtered_annotations
                ]
