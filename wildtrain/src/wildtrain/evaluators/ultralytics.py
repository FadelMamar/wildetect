...
class UltralyticsEvaluator:
    ...
    def __init__(
        self, config: DetectionEvalConfig, disable_detection_filtering: bool = False
    ):
        self.config = config
    ...
    def _get_metrics(
        ...
        average = getattr(
            sv.metrics.core.AveragingMethod, self.config.metrics.average.upper()
        )
        return dict(
    ...
    def _compute_metrics(self, results: Dict[str, List[sv.Detections]]) -> None:
        ...
        assert len(results["predictions"]) == len(results["ground_truth"]), (
            f"Number of predictions and ground truth must be the same, got {len(results['predictions'])} and {len(results['ground_truth'])}"
        )
        for metric_name, metric in self.metrics.items():
            for pred, gt in zip(results["predictions"], results["ground_truth"]):
                if not isinstance(pred, sv.Detections):
                    raise ValueError(
                        f"Prediction must be a sv.Detections object, got {type(pred)}"
                    )
                if not isinstance(gt, sv.Detections):
                    raise ValueError(
                        f"Ground truth must be a sv.Detections object, got {type(gt)}"
                    )
    ...
    def _load_model(self, disable_detection_filtering: bool = False) -> Detector:
        ...
        return Detector.from_config(
            ...
            classifier_ckpt=self.config.weights.classifier,
        )
    ...
    def _collate_fn(
        self, batch: List[Tuple[str, np.ndarray, sv.Detections]]
    ) -> Dict[str, Any]:
        """
    ...
    def _filter_annotation(self, annotation: sv.Detections) -> sv.Detections:
        ...
        if self.config.eval.single_cls:
            filtered_annotation.class_id = np.zeros_like(
                filtered_annotation.class_id, dtype=int
            )
