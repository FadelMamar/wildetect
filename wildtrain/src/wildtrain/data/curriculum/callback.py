...
class CurriculumCallback(L.Callback):
            ...
            if trainer.logger:
                ...
                if initial_state:
                    trainer.logger.log_metrics(
                        {
                            "curriculum/initial_difficulty": initial_state.get(
                                "difficulty", 0.0
                            ),
                        },
                        step=0,
    ...
    def on_train_epoch_start(
        ...
        curriculum_state = self.datamodule.update_curriculum_epoch(
    ...
    def _log_curriculum_metrics(
        self, trainer: L.Trainer, curriculum_state: Dict[str, Any]
    ) -> None:
        ...
        if trainer.current_epoch % self.log_frequency == 0:
            ...
            if (
                hasattr(self.datamodule, "curriculum_config")
                and self.datamodule.curriculum_config
            ):
    ...
    def on_train_epoch_end(
        ...
        if trainer.logger and trainer.current_epoch % self.log_frequency == 0:
            ...
            if current_state:
                trainer.logger.log_metrics(
                    {
                        "curriculum/epoch_end_difficulty": current_state.get(
                            "difficulty", 0.0
                        ),
                    },
