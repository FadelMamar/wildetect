...
class Detector(torch.nn.Module):
    ...
    def __init__(
        self, localizer: ObjectLocalizer, classifier: Optional[GenericClassifier] = None
    ):
        super().__init__()
    ...
    def from_config(
        ...
        if classifier_ckpt is not None:
            assert isinstance(classifier_ckpt, (Path, str)), (
                "classifier_ckpt must be a Path object or a string"
            )
            classifier = GenericClassifier.load_from_checkpoint(
                classifier_ckpt, map_location=localizer_config.device
            )
            if classifier_export_kwargs is not None:
                assert isinstance(
                    classifier_export_kwargs, (RegistrationBase, DictConfig)
                ), (
                    f"classifier_export_kwargs must be a RegistrationBase object. Received {type(classifier_export_kwargs)}"
                )
                export_path = (
                    Path(classifier_ckpt)
                    .with_suffix(f".{classifier_export_kwargs.export_format}")
                    .as_posix()
                )
                classifier = classifier.export(
                    mode=classifier_export_kwargs.export_format,
    ...
    def _pad_if_needed(self, batch: torch.Tensor) -> torch.Tensor:
        if self.input_shape is None:
            return batch
        assert len(self.input_shape) == len(batch.shape), (
            f"Expected {len(self.input_shape)}D tensor but received {len(batch.shape)}D tensor"
        )
        assert all([a <= b for a, b in zip(batch.shape[1:], self.input_shape[1:])]), (
            f"Expected {self.input_shape[1:]} but received {batch.shape[1:]}"
        )
    ...
    def resize_bbox(
        self, bbox: np.ndarray, roi_size: int, images_width: int, images_height: int
    ) -> np.ndarray:
        ...
        for result in results:
            ...
            for k, v in result.items():
                if isinstance(v, np.ndarray):
                    ...
                    result_as_dict[k] = {
                        k: v[k].tolist() if isinstance(v[k], np.ndarray) else v[k]
                        for k in v
                    }
                else:
                    result_as_dict[k] = v
        ...
        for result in results:
            detections.append(
                sv.Detections(
                    ...
                    metadata={
                        "class_mapping": result["metadata"].get("class_mapping", {})
                    },
                )
            )
        return detections
    ...
    @staticmethod
    def to_fiftyone(
        detections: List[sv.Detections], images_width: int, images_height: int
    ) -> List[fo.Detection]:
        fo_detections = []
    ...
    def predict_inference_service(
        batch: torch.Tensor,
        url: str = "http://localhost:4141/predict",
        timeout: int = 15,
    ) -> List[sv.Detections]:
        ...
        res = requests.post(url=url, json=payload, timeout=timeout).json()
    ...
    def predict_dataset(
        ...
        assert tensor_dataset.ndim == 4, (
            f"Expected 4D tensor but received {tensor_dataset.ndim}D tensor"
        )
        assert tensor_dataset.shape[1] == c, (
            f"Expected {c} channels but received {tensor_dataset.shape[1]} channels"
        )
        assert tensor_dataset.shape[0] > b, (
            f"Expected {tensor_dataset.shape[0]} > {b}, number of images in the dataset must be greater than the batch size"
        )
        detections = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    self.predict,
                    tensor_dataset[i : i + b],
                    return_as_dict=return_as_dict,
                )
                for i in range(0, tensor_dataset.shape[0], b)
            ]
