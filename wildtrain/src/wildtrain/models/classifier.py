...
class ResizeNormalize(nn.Module):
    def __init__(self, input_size: int, mean: torch.Tensor, std: torch.Tensor):
        ...
        assert isinstance(self.input_size, int), (
            f"input_size must be an integer, received {type(self.input_size)}"
        )
...
class GenericClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.fc = ClassifierHead(
            input_dim=self.feature_extractor.num_features,
        ...
        self.preprocessing = ResizeNormalize(
            input_size=self.input_size.item(), mean=self.mean, std=self.std
        )
    ...
    def set_onnx_program(self, onnx_program: Optional[ort.InferenceSession] = None):
    ...
    def export(
        self, mode: str, output_path: str, batch_size: int = 8
    ) -> "GenericClassifier":
        if mode == "torchscript":
    ...
    def to_onnx(
        ...
        # export program
        dynamic_shapes = {"x": {0: Dim("batch_size", min=1, max=b), 1: c, 2: h, 3: w}}
        ...
        self._onnx_program = ort.InferenceSession(
            output_path, providers=self._onnx_providers
        )
        return self

    def _export_onnx(
        self, shape: Tuple[int, int, int, int], report: bool = False
    ) -> torch.onnx.ONNXProgram:
        b, c, h, w = shape
        x = torch.rand(b, c, h, w, dtype=torch.float32)
        cfg = dict(
            dynamic_shapes={
                "input": {0: Dim("batch_size", min=1, max=b)},
    ...
    def _save_onnx_program(
        ...
        try:
            ...
            logger.error(
                f"Failed to save ONNX program to {output_path}. {traceback.format_exc()}"
            )

    def _predict_onnx(self, x: torch.Tensor) -> torch.Tensor:
        assert isinstance(self._onnx_program, ort.InferenceSession), (
            "ONNX program not created. Call to_onnx() first."
        )
    ...
    def predict(self, x: torch.Tensor) -> list[dict]:
        ...
        classes = [self.class_mapping.get(i, "None") for i in labels]
        scores = probs.max(dim=1).values.cpu().tolist()
        return [
            {"class": c, "score": s, "class_id": l}
            for c, s, l in zip(classes, scores, labels)
        ]
    ...
    def load_from_checkpoint(
        ...
        label_to_class_map: Optional[dict] = None,
    ):
        if str(checkpoint_path).endswith(".onnx"):
            assert isinstance(label_to_class_map, dict), (
                "label_to_class_map must be provided if checkpoint_path is an ONNX file"
            )
            return cls._load_onnx(
                onnx_path=checkpoint_path, label_to_class_map=label_to_class_map
            )
        try:
            return cls._load_from_lightning_ckpt(
                checkpoint_path=checkpoint_path, map_location=map_location
            )
        except Exception as e:
            logger.error(
                f"Failed to load checkpoint from {checkpoint_path} using lightning. {e}"
            )
            return cls._load_from_checkpoint(
    ...
    def _load_from_lightning_ckpt(cls, checkpoint_path: str, map_location: str = "cpu"):
        ...
        if "state_dict" in checkpoint and "hyper_parameters" in checkpoint:
            ...
            raise ValueError(
                "Checkpoint does not appear to be a valid PyTorch Lightning checkpoint"
            )
    ...
    def _load_from_checkpoint(
        ...
        if state_dict is None:
            assert checkpoint_path is not None, (
                "checkpoint_path must be provided if state_dict is not provided"
            )
    ...
    @classmethod
    def _load_onnx(
        cls, onnx_path: str, label_to_class_map: dict
    ) -> "GenericClassifier":
