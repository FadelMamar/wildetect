...
class PipelineConfig(BaseConfig):
    ...
    def validate_pipeline_configs(cls, v, info):
        ...
        if hasattr(info.data, "disable_train") and hasattr(info.data, "disable_eval"):
            if info.data.disable_train and info.data.disable_eval:
                raise ValueError(
                    "At least one pipeline (train or eval) must be enabled"
                )
        return v
    ...
    def validate_pipeline_files_exist(self) -> None:
        ...
        if self.is_train_enabled() and not Path(self.train.config).exists():
            raise ValueError(
                f"Training config file does not exist: {self.train.config}"
            )
        if self.is_eval_enabled() and not Path(self.eval.config).exists():
            raise ValueError(
                f"Evaluation config file does not exist: {self.eval.config}"
            )
...
class ClassificationPipelineConfig(PipelineConfig):
    ...
    def get_classification_results_path(self) -> str:
        ...
        return (
            Path(self.results_dir) / "classification"
            if Path(self.results_dir).name != "classification"
            else self.results_dir
        )

    @classmethod
    def create_default(
        cls, results_dir: str = str("results/classification")
...
class DetectionPipelineConfig(PipelineConfig):
    ...
    def get_detection_results_path(self) -> str:
        ...
        return (
            ...
            if self.results_dir.name != "detection"
            else self.results_dir
        )

    @classmethod
    def create_default(
        cls, results_dir: str = str("results/yolo")
    ) -> "DetectionPipelineConfig":
        """Create a default detection pipeline configuration."""
        return cls(
            disable_train=False,
            train=TrainPipelineConfig(
                config=str("configs/detection/yolo_configs/yolo.yaml"), debug=False
            ),
            disable_eval=True,
            eval=EvalPipelineConfig(
                config=str("configs/detection/yolo_configs/yolo_eval.yaml"), debug=False
            ),
            results_dir=results_dir,
        )
