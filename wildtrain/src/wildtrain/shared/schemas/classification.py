...
class ClassificationDatasetConfig(BaseConfig):
    ...
    curriculum_config: Optional[CurriculumConfig] = Field(
        default=None, description="Curriculum configuration"
    )
    single_class: Optional[SingleClassConfig] = Field(
        default=None, description="Single class configuration"
    )
    ...
    crop_size: Optional[int] = Field(
        default=None, gt=0, description="Crop size for crop datasets"
    )
    max_tn_crops: Optional[int] = Field(
        default=None, gt=0, description="Maximum true negative crops"
    )
    p_draw_annotations: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Probability of drawing annotations"
    )
    compute_difficulties: Optional[bool] = Field(
        default=None, description="Compute crop difficulties"
    )
    preserve_aspect_ratio: Optional[bool] = Field(
        default=None, description="Preserve aspect ratio"
    )
...
class ClassifierTrainingConfig(BaseConfig):
    ...
    label_smoothing: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Label smoothing"
    )
    ...
    val_check_interval: int = Field(
        default=1, ge=1, description="Validation check interval"
    )
...
class ClassifierCheckpointConfig(BaseConfig):
    ...
    save_top_k: int = Field(
        default=1, ge=0, description="Number of best models to save"
    )
...
class ClassificationConfig(BaseConfig):
    ...
    checkpoint: ClassifierCheckpointConfig = Field(
    ...
    def validate_model_dataset_compatibility(cls, v, info):
        if hasattr(info.data, "dataset"):
            ...
            if v.input_size != dataset.input_size:
                raise ValueError(
                    f"Model input_size ({v.input_size}) must match dataset input_size ({dataset.input_size})"
                )
        return v
...
class ClassificationEvalDatasetConfig(BaseConfig):
    ...
    root_data_directory: str = Field(
        description="Root directory containing the dataset"
    )
    single_class: Optional[SingleClassConfig] = Field(
        default=None, description="Single class configuration"
    )
...
class ClassificationEvalConfig(BaseConfig):
    ...
    split: Literal["train", "val", "test"] = Field(
        default="val", description="Dataset split to evaluate on"
    )
    device: str = Field(
        default="cpu", description="Device to run evaluation on (cpu, cuda)"
    )
    batch_size: int = Field(default=4, description="Batch size for evaluation")
    dataset: ClassificationEvalDatasetConfig = Field(
