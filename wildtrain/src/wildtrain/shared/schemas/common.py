...
class LoggingConfig(BaseConfig):
    ...
    experiment_name: str = Field(
        default="wildtrain", description="MLflow experiment name"
    )
...
class TransformsConfig(BaseConfig):
    ...
    train: List[TransformConfig] = Field(
        default_factory=list, description="Training transforms"
    )
    val: List[TransformConfig] = Field(
        default_factory=list, description="Validation transforms"
    )
...
class CurriculumConfig(BaseConfig):
    ...
    type: Literal["difficulty"] = Field(
        default="difficulty", description="Curriculum type"
    )
    difficulty_strategy: Literal["linear"] = Field(
        default="linear", description="Difficulty strategy"
    )
    start_difficulty: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Starting difficulty"
    )
    end_difficulty: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Ending difficulty"
    )
...
class SingleClassConfig(BaseConfig):
    ...
    keep_classes: Optional[List[str]] = Field(
        default=None, description="Classes to keep (if None, all classes kept)"
    )
    discard_classes: Optional[List[str]] = Field(
        default=None, description="Classes to discard"
    )
