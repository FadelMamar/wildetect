...
class DetectionConfig(BaseConfig):
    ...
    curriculum: YoloCurriculumConfig = Field(
    ...
    custom_yolo_kwargs: YoloCustomConfig = Field(
        description="Custom YOLO configuration"
    )
...
class DetectionWeightsConfig(BaseConfig):
    ...
    classifier: Optional[str] = Field(
        default=None, description="str to the classifier weights file (optional)"
    )
...
class DetectionMetricsConfig(BaseConfig):
    ...
    average: Literal["macro", "micro", "weighted"] = Field(
        default="macro", description="Averaging method for metrics"
    )
    class_agnostic: bool = Field(
        default=False, description="Whether to use class-agnostic evaluation"
    )
...
class DetectionEvalParamsConfig(BaseConfig):
    ...
    split: Literal["train", "val", "test"] = Field(
        default="val", description="Split to evaluate on"
    )
    ...
    task: Literal["detect", "classify", "segment"] = Field(
        default="detect", description="Task type"
    )
    classes: Optional[List[int]] = Field(
        default=None, description="Optionally restrict to specific class indices"
    )
    ...
    overlap_metric: OverlapMetricConfig = Field(
        default=OverlapMetricConfig.IOU, description="Overlap metric for evaluation"
    )
...
class DetectionEvalConfig(BaseConfig):
    ...
    metrics: DetectionMetricsConfig = Field(
    ...
    def validate_device(cls, v):
        assert (v == "cpu") or ("cuda" in v), (
            f"Device must be one of ['cpu', 'cuda'], got: {v}"
        )
        return v
