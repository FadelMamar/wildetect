...
class ClassifierRegistrationConfig(BaseConfig):
    ...
    weights: Optional[str] = Field(
        default=None, description="Path to the model checkpoint file"
    )
    processing: RegistrationBase = Field(description="processing information")
...
class DetectorRegistrationConfig(BaseConfig):
    ...
    localizer: LocalizerRegistrationConfig = Field(
        description="Detector registration configuration"
    )
    classifier: ClassifierRegistrationConfig = Field(
        description="Classifier registration configuration"
    )
    processing: RegistrationBase = Field(description="processing information")
...
class InferenceConfig(BaseConfig):
    ...
    workers_per_device: int = Field(
        default=1, description="Number of workers per device"
    )
    mlflow_registry_name: str = Field(
        default="detector", description="MLflow registry name"
    )
    mlflow_alias: str = Field(default="demo", description="MLflow alias")
    mlflow_local_dir: str = Field(
        default="models-registry", description="MLflow local directory"
    )
    mlflow_tracking_uri: str = Field(
        default="http://localhost:5000", description="MLflow tracking server URI"
    )
