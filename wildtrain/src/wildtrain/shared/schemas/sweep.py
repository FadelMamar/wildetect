...
class SweepOutputConfig(BaseConfig):
    ...
    directory: Optional[str] = Field(
        default=None,
        description="Output directory for results (defaults to results/sweeps/sweep_name)",
    )
    ...
    include_optimization_history: bool = Field(
        default=True, description="Include all trials in output"
    )
    ...
    def validate_dropout(cls, v):
        ...
        for value in v:
            if not 0.0 <= value <= 1.0:
                raise ValueError(
                    f"dropout values must be between 0.0 and 1.0, got {value}"
                )
        return v
...
class ClassificationSweepTrainParametersConfig(BaseConfig):
    ...
    label_smoothing: List[float] = Field(
        description="List of label smoothing values to search"
    )
    weight_decay: List[float] = Field(
        description="List of weight decay values to search"
    )
    ...
    def validate_label_smoothing(cls, v):
        ...
        for value in v:
            if not 0.0 <= value <= 1.0:
                raise ValueError(
                    f"label_smoothing values must be between 0.0 and 1.0, got {value}"
                )
        return v
    ...
    def validate_weight_decay(cls, v):
        ...
        for value in v:
            if value < 0.0:
                raise ValueError(
                    f"weight_decay values must be non-negative, got {value}"
                )
        return v
    ...
    def validate_batch_size(cls, v):
        ...
        for value in v:
            if value <= 0:
                raise ValueError(
                    f"batch_size values must be greater than 0, got {value}"
                )
        return v
...
class ClassificationSweepParametersConfig(BaseConfig):
    ...
    model: ClassificationSweepModelParametersConfig = Field(
        description="Model parameters to search"
    )
    train: ClassificationSweepTrainParametersConfig = Field(
        description="Training parameters to search"
    )
...
class ClassificationSweepConfig(BaseConfig):
    ...
    parameters: ClassificationSweepParametersConfig = Field(
...
class DetectionSweepModelParametersConfig(BaseConfig):
    ...
    architecture_file: Optional[List[str]] = Field(
        default=None, description="List of architecture files to search"
    )
    weights: Optional[List[str]] = Field(
        default=None, description="List of weight files to search"
    )
...
class DetectionSweepTrainParametersConfig(BaseConfig):
    ...
    box: Optional[List[float]] = Field(
        default=None, description="List of box loss weights to search"
    )
    cls: Optional[List[float]] = Field(
        default=None, description="List of class loss weights to search"
    )
    dfl: Optional[List[float]] = Field(
        default=None, description="List of DFL loss weights to search"
    )
    ...
    def validate_optimizer(cls, v):
        ...
        for value in v:
            if value not in valid_optimizers:
                raise ValueError(
                    f"optimizer must be one of {valid_optimizers}, got {value}"
                )
        return v
    ...
    def validate_loss_weights(cls, v):
        ...
        if v is not None:
            for value in v:
                if value < 0.0:
                    raise ValueError(
                        f"loss weight values must be non-negative, got {value}"
                    )
        return v
...
class DetectionSweepParametersConfig(BaseConfig):
    ...
    model: Optional[DetectionSweepModelParametersConfig] = Field(
        default=None, description="Model parameters to search"
    )
    train: DetectionSweepTrainParametersConfig = Field(
        description="Training parameters to search"
    )
...
class DetectionSweepConfig(BaseConfig):
    ...
    base_config: str = Field(description="Path to base training configuration file")
    parameters: DetectionSweepParametersConfig = Field(
        description="Hyperparameter search space"
    )
    sweep_name: str = Field(description="Name of the sweep experiment")
    n_trials: int = Field(gt=0, le=1000, description="Number of optimization trials")
    objective: SweepObjectiveTypes = Field(
        default=SweepObjectiveTypes.MAP_50, description="Objective to optimize"
    )
    direction: SweepDirectionTypes = Field(
        default=SweepDirectionTypes.MAXIMIZE, description="Direction to optimize"
    )
    seed: int = Field(description="Random seed for reproducibility")
    timeout: Optional[int] = Field(
        default=None, description="Maximum time for optimization in seconds"
