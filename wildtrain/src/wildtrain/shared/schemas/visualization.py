...
class FiftyOneConfig(BaseConfig):
    ...
    prediction_field: Optional[str] = Field(
        default=None, description="Prediction field name"
    )
...
class DetectionVisualizationConfig(BaseConfig):
    ...
    classifier_weights: Optional[str] = Field(
        default=None, description="Classifier weights path"
    )
...
class ClassificationVisualizationConfig(BaseConfig):
    ...
    dataset_name: str = Field(
        description="Name of the FiftyOne dataset to use or create"
    )
    weights: str = Field(description="str to the classifier checkpoint (.ckpt) file")
    prediction_field: str = Field(
        default="classification_predictions",
        description="Field name to store predictions in FiftyOne samples",
    )
    batch_size: int = Field(
        default=32, description="Batch size for prediction inference"
    )
    device: str = Field(
        default="cpu", description="Device to run inference on (e.g., 'cpu' or 'cuda')"
    )
    debug: bool = Field(
        default=False,
        description="If set, only process a small number of samples for debugging",
    )
    mlflow: MLflowConfig = Field(description="MLflow configuration")
    label_to_class_map: Optional[dict] = Field(
        default=None, description="Label to class map"
    )
