...
def classifier(
    config: Path = typer.Option(
        ...
        "--config",
        "-c",
        help="Path to classifier registration configuration file",
    ),
    weights_path: Path = typer.Option(
        None, "--weights-path", help="Path to the model checkpoint file"
    ),
    name: str = typer.Option(
        "classifier", "--name", "-n", help="Model name for registration"
    ),
    batch_size: int = typer.Option(
        8, "--batch-size", "-b", help="Batch size for inference"
    ),
    mlflow_tracking_uri: str = typer.Option(
        "http://localhost:5000", "--mlflow-uri", help="MLflow tracking server URI"
    ),
) -> None:
    ...
    if config is not None:
        ...
        console.print(
            f"[bold green]Loading classifier registration config from:[/bold green] {config}"
        )
        try:
            cfg = ConfigLoader.load_classifier_registration_config(config)
            console.print(
                "[bold green]✓[/bold green] Classifier registration configuration validated successfully"
            )
        ...
        if weights_path is None:
            console.print(
                "[bold red]✗[/bold red] Must provide either --config or --weights-path"
            )
            raise typer.Exit(1)

    console.print(
        f"[bold green]Registering classifier model:[/bold green] {weights_path}"
    )
    ...
    try:
        ...
        registrar.register_classifier(
            weights=weights_path, name=name, batch_size=batch_size
        )

        console.print(
            f"[bold green]✓[/bold green] Successfully registered classifier model: {name}"
        )
        console.print(
            f"[bold blue]Model registered to MLflow tracking URI:[/bold blue] {mlflow_tracking_uri}"
        )

    except FileNotFoundError:
        console.print(
            f"[bold red]✗[/bold red] Model weights not found: {traceback.format_exc()}"
        )
        raise typer.Exit(1)
    except Exception:
        console.print(
            f"[bold red]✗[/bold red] Registration failed: {traceback.format_exc()}"
        )
        if log_file.exists():
...
def detector(
    ...
    try:
        ...
        # Register the classifier
        registrar.register_detector(config_path=config)
        cfg = ConfigLoader.load_detector_registration_config(config)
        console.print("Configuration:", cfg)
        console.print(
            "[bold green]✓[/bold green] Successfully registered detector model."
        )
