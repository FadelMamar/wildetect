...
class Visualizer:
    def __init__(
        ...
        assert (fiftyone_dataset_name is None) ^ (label_studio_url is None), (
            "Either fiftyone_dataset_name or label_studio_url must be provided"
        )
        ...
        if fiftyone_dataset_name is not None:
            ...
            self.project = self.label_studio_client.projects.get(
                id=label_studio_project_id
            )
    ...
    def add_predictions_from_classifier(
        self,
        ...
        for batch_tensor, batch_samples in self.get_fiftyone_samples(
            ...
            for sample, pred in zip(batch_samples, preds):
                if pred is not None:
                    sample[prediction_field] = fo.Classification(
                        label=pred["class"], confidence=pred["score"]
                    )
                    sample.save()
    ...
    def add_predictions_from_detector(
        ...
        def normalize_and_resize(batch_tensor: torch.Tensor):
            ...
            batch_tensor = T.Resize(
                (imgsz, imgsz), interpolation=T.InterpolationMode.NEAREST
            )(batch_tensor)
            return batch_tensor
        ...
        if self.fiftyone_dataset is not None:
            for batch_tensor, batch_samples in self.get_fiftyone_samples(
                ...
                for sample, detections in zip(batch_samples, detections_list):
                    if detections is None or len(detections) == 0:
                        continue
                    fo_detections = Detector.to_fiftyone(
                        detections, images_width=W, images_height=H
                    )
            ...
            for batch_tensor, task_ids in self.get_label_studio_samples(
                debug=debug, batch_size=batch_size
            ):
                batch_tensor = normalize_and_resize(batch_tensor)
                B, C, H, W = batch_tensor.shape
