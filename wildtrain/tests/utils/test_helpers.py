...
def create_temp_dataset(
    num_images: int = 10, image_size: Tuple[int, int] = (224, 224)
) -> str:
...
def create_mock_classification_annotations(
    num_samples: int = 20, num_classes: int = 3
) -> List[Dict]:
...
def create_mock_detection_annotations(
    num_samples: int = 10, num_classes: int = 3
) -> Dict:
...
def validate_class_distribution(
    annotations: List[Dict], expected_classes: int = None
) -> Dict[int, int]:
    ...
    if expected_classes is not None:
        assert len(class_counts) == expected_classes, (
            f"Expected {expected_classes} classes, got {len(class_counts)}"
        )
...
def validate_crop_dataset_structure(crop_dataset, expected_crops: int = None):
    ...
    assert hasattr(crop_dataset, "__getitem__"), (
        "PatchDataset must have __getitem__ method"
    )

    if expected_crops is not None:
        assert len(crop_dataset) == expected_crops, (
            f"Expected {expected_crops} crops, got {len(crop_dataset)}"
        )
    ...
    if len(crop_dataset) > 0:
        ...
        assert isinstance(sample, (tuple, list)), (
            "PatchDataset should return tuple/list"
        )
...
def calculate_class_distribution_change(
    before_annotations: List[Dict], after_annotations: List[Dict]
) -> Dict:
...
def create_test_config(config_type: str = "classification") -> Dict[str, Any]:
    ...
    base_config = {
        "data": {"batch_size": 4, "num_workers": 0, "debug": True},
        "training": {"max_epochs": 2, "debug": True},
    }

    if config_type == "classification":
        base_config.update({"model": {"backbone": "resnet18", "num_classes": 3}})
    elif config_type == "detection":
        base_config.update(
            {"model": {"weights": "yolov8n.pt", "imgsz": 224, "device": "cpu"}}
        )
...
def assert_list_length(
    actual_list: List, expected_length: int, list_name: str = "list"
):
    """Assert that a list has the expected length."""
    assert len(actual_list) == expected_length, (
        f"Expected {list_name} to have length {expected_length}, got {len(actual_list)}"
    )


def assert_dict_keys(
    actual_dict: Dict, expected_keys: List[str], dict_name: str = "dictionary"
):
