...
def detect_cuda_version() -> Optional[str]:
    ...
    try:
        ...
        logger.info(
            f"Mapped driver version {driver_major} to CUDA version {cuda_version}"
        )
        return cuda_version
...
def check_cuda_availability() -> bool:
    ...
    try:
        run_subprocess_with_logging(["nvidia-smi"], "Checking CUDA availability")
...
def install_cuda_torch(cuda_version: Optional[str] = None) -> None:
    ...
    try:
        ...
        run_subprocess_with_logging(
            ...
            "Uninstalling existing torch packages",
        ...
        if cuda_version == "121":
            ...
            logger.warning(
                f"Unknown CUDA version '{cuda_version}', falling back to CPU-only"
            )
            print(
                f"⚠️  Unknown CUDA version '{cuda_version}', installing CPU-only PyTorch."
            )
            install_cpu_torch()
            return
...
def verify_cuda_installation() -> None:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logger.warning(
                "CUDA is not available in PyTorch despite installation attempt"
            )
...
def main():
    ...
    logger.info(
        f"Arguments: cuda_version={args.cuda_version}, cpu_only={args.cpu_only}"
    )
