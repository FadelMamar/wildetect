...
class IoUTuner:
    ...
    def __init__(
        ...
        # Separate predictions and groundtruth
        self.df_preds = df_annotations[df_annotations["origin"] != "manual"].copy()
        self.df_gt = df_annotations[df_annotations["origin"] == "manual"].copy()
    ...
    def _compute_metrics(
        ...
        for task_id in self.df["task_id"].unique():
            ...
            tp, fp, fn = self._match_detections(preds, gt, match_iou_threshold)
            total_tp += tp
            total_fp += fp
            total_fn += fn
