...
class Detection:
    ...
    # Optional GPS fields
    geographic_footprint: Optional[Any] = None  # GeographicBounds
    ...
    def __post_init__(self):
        ...
        assert isinstance(self.bbox, list) and len(self.bbox) == 4, f"bbox must be a list of 4 elements, but got {self.bbox}"
        assert all([isinstance(coord, int) for coord in self.bbox]), f"bbox must be a list of integers, but got {self.bbox}"
    ...
    @classmethod
    def from_fiftyone(cls, fo_detection, class_id: Optional[int] = None) -> Optional['Detection']:
        ...
        try:
            ...
            logger.warning("FiftyOne not available, cannot convert from FiftyOne format")
            return None
    
    def set_gps_location(self, gps_loc: str,) -> None:
        """Set the GPS location for this detection.
        
        Args:
            gps_loc (str): GPS location string
        """
        assert isinstance(gps_loc, str), f"gps_loc must be a string, but got {type(gps_loc)}"
    ...
    def set_geographic_footprint(self, geographic_footprint: GeographicBounds) -> None:
        ...
        """
        assert isinstance(geographic_footprint, GeographicBounds), f"geographic_footprint must be a GeographicBounds, but got {type(geographic_footprint)}"
    ...
    def has_gps_data(self) -> bool:
        ...
        """
        return (self.gps_loc is not None or 
                self.image_gps_loc is not None or 
                self.geographic_footprint is not None)
