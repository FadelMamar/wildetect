...
def get_processor(name: str) -> type:
    ...
    if name not in processor_map:
        raise NotImplementedError(f"Processor '{name}' is not implemented. Available: {list(processor_map.keys())}")
...
class Processor(ABC):
    ...
    def _validate_config(self):
        ...
        if not isinstance(self.config, ProcessorConfig):
            raise ValueError("config must be a ProcessorConfig instance")
...
class FeatureExtractor(Processor):
    ...
    def __init__(
        ...
        try:
            ...
            self.logger.info(f"Feature extractor initialized with model: {hf_model_path}")
    ...
    def run(self, images: Sequence[Image.Image]) -> np.ndarray:
        ...
        try:
            ...
            for i in range(0, len(images), batch_size):
                ...
                features = outputs.pooler_output.cpu().reshape(len(batch_images), -1).numpy()
...
class Classifier(Processor):
    ...
    def run(self, images: Sequence[Image.Image]) -> List[str]:
        ...
        try:
            ...
            predictions = map(lambda x: self.label_map[x], pred)
...
class RoIPostProcessor(Processor):
    ...
    def __init__(
        ...
        self.logger.info(f"DetectionsPostprocessor initialized to keep classes: {self.keep}")
        ...
        try:
            ...
            for det in detections:
                if not hasattr(det, 'x_center') or not hasattr(det, 'y_center'):
                    self.logger.warning(f"Detection missing x_center/y_center coordinates: {det}")
                    continue
                ...
                if x2 - x1 < 10 or y2 - y1 < 10:
                    self.logger.warning(f"Detection crop too small: {x1},{y1},{x2},{y2}")
                    continue
            ...
            filtered_detections = [
                det for det, pred in zip(valid_detections, preds) 
                if pred in self.keep
            ]
            
            self.logger.info(f"Filtered {len(detections)} -> {len(filtered_detections)} detections")
